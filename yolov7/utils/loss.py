...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        # Define criteria
        # BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([model.hyp['cls_pw']], device=device))
        # BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([model.hyp['obj_pw']], device=device))
        BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([model.hyp['cls_pw']]))
        BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([model.hyp['obj_pw']]))

        # Class label smoothing https://arxiv.org/pdf/1902.04103.pdf eqn 3
        self.cp, self.cn = smooth_BCE(eps=model.hyp.get('label_smoothing', 0.0))  # positive, negative BCE targets
        ...
        self.BCEcls, self.BCEobj, self.gr, self.hyp, self.autobalance = BCEcls, BCEobj, model.gr, model.hyp, autobalance
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        self.BCEcls, self.BCEobj = self.BCEcls.to(device), self.BCEobj.to(device)
